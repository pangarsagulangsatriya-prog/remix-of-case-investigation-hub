...
# initialAgentsState starts at line 188 (0-indexed 187)
# It ends around line 465 (0-indexed 464)
# Let's find the exact range
actor_start = -1
actor_end = -1
for i in range(180, 500):
    if "id: 'actor'" in lines[i]:
        actor_start = i
        # Find the start of the object {
        while actor_start > 0 and '{' not in lines[actor_start]:
            actor_start -= 1
        # Find the end of the object },
        actor_end = i
        brace_count = 1
        while brace_count > 0 and actor_end < len(lines):
            actor_end += 1
            brace_count += lines[actor_end].count('{')
            brace_count -= lines[actor_end].count('}')
        actor_end += 1 # Include the trailing comma if any
        break

if actor_start != -1:
    print(f"Removing actor from line {actor_start+1} to {actor_end+1}")
    del lines[actor_start:actor_end]
    ...
    if "id: 'peepo'" in lines[i]:
        for j in range(i, i+10):
            if "dependencies: ['actor']" in lines[j]:
                ...
                print(f"Updated peepo dependency at line {j+1}")
                break
        break

# 3. Update startFullChain
for i in range(len(lines)):
    if "const startFullChain = () => {" in lines[i]:
        for j in range(i, i+10):
            if 'setChainQueue(["fact", "actor", "peepo", "ipls", "prev"]);' in lines[j]:
                lines[j] = lines[j].replace('"actor", ', '')
                print(f"Updated startFullChain at line {j+1}")
                break
        break

# 4. Replace AnalysisTab return statement
# AnalysisTab starts at 3727
# return starts around 4016
# ReportsTab starts at 5265

analysis_return_start = -1
for i in range(3700, 4100):
    if "return (" in lines[i]:
        analysis_return_start = i
        break

reports_tab_start = -1
for i in range(analysis_return_start, len(lines)):
    if "function ReportsTab()" in lines[i]:
        reports_tab_start = i
        break

if analysis_return_start != -1 and reports_tab_start != -1:
    # Find the last ); } before ReportsTab
    analysis_return_end = reports_tab_start - 1
    while analysis_return_end > analysis_return_start and "}" not in lines[analysis_return_end]:
        analysis_return_end -= 1
    analysis_return_end += 1
    
    print(f"Replacing AnalysisTab return from line {analysis_return_start+1} to {analysis_return_end+1}")
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.writelines(lines[:analysis_return_start] + [new_jsx] + lines[analysis_return_end:])
