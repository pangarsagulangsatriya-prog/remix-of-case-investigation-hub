...
# 4. Fix the Return Statement
# We will use a more surgical approach to avoid the str/list error and ensure correctness.

# First, let's find the start of the return statement
start_marker = 'return ('
# Find the one after line 4000
search_start = content.find(start_marker, 4000)
if search_start == -1:
    print("Could not find start of return statement")
    exit(1)

# Now find the end of the return statement (the last ) before function ReportsTab)
end_marker = 'function ReportsTab()'
search_end = content.find(end_marker, search_start)
if search_end == -1:
    print("Could not find ReportsTab")
    exit(1)

# Find the last ); before ReportsTab
last_brace = content.rfind(');', search_start, search_end)
if last_brace == -1:
    print("Could not find end of return statement")
    exit(1)

# Construct the new JSX
# This JSX preserves the user's requested agent card styling and removes actor.
new_jsx = """
                  ...
                  {agents.map((agent) => (
                     ...
                     >
                        <div className="flex items-start justify-between mb-4">
                           <div className={`h-12 w-12 rounded-sm border flex items-center justify-center transition-all ${selectedAgentId === agent.id ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-400 border-slate-100"}`}>
...
final_content = content[:search_start] + start_marker + new_jsx + ");\n}" + content[search_end:]
