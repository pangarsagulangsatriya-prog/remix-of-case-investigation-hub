...
for idx, line in enumerate(lines):
    if "return (" in line and idx > 3900:
        start_index = idx
        break
...
# The end of the return statement is a few lines before ReportsTab
# Usually it's:
#    );
# }
#
...
# Let's adjust end_index to be just before function ReportsTab
# and look for the component closing brace.

# We will replace from start_index to end_index-1 (approx)
                  ...
                  {agents.map((agent) => (
                     ...
                     >
                        ...
                        <div className="space-y-1 mb-4">
                           <h3 className="text-[11px] font-black text-slate-900 uppercase tracking-wider">{agent.name}</h3>
                           <p className="text-[9px] font-bold text-slate-400 uppercase leading-tight line-clamp-2">{agent.purpose}</p>
                        </div>
                                    ...
                                    <TooltipProvider key={depId}>
                                       <Tooltip>
                                          <TooltipTrigger asChild>
                                             <div className="h-6 w-6 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-slate-400">
                                                <dep.icon className="h-3 w-3" />
                                             </div>
                                          </TooltipTrigger>
                                          <TooltipContent>
                                             <p className="text-[10px] font-bold uppercase tracking-widest">Depends on {dep.name}</p>
                                          </TooltipContent>
                                       </Tooltip>
                                    </TooltipProvider>
                                 );
...
final_lines = lines[:start_index] + [new_return_content] + lines[end_index:]
